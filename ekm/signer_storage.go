...
func NewSignerStorage(db basedb.IDb, network beacon.Network, logger *zap.Logger, password []byte) *storage {
	return &storage{
		db:       db,
		network:  network,
...
func (s *storage) ListAccounts() ([]core.ValidatorAccount, error) {
	...
	err := s.db.GetAll(s.logger, s.objPrefix(accountsPrefix), func(i int, obj basedb.Obj) error {
		decrypted, err := Decrypt(s.password, obj.Value)
		if err != nil {
			return err
		}
		acc, err := s.decodeAccount(decrypted)
		if err != nil {
...
func (s *storage) SaveAccount(account core.ValidatorAccount) error {
	...
	encrypted, err := Encrypt(s.password, data)
	if err != nil {
		return err
	}
	...
	return s.db.Set(s.objPrefix(accountsPrefix), []byte(key), encrypted)
}
...
func (s *storage) OpenAccount(accountID uuid.UUID) (core.ValidatorAccount, error) {
	...
	return s.decodeAccount(decrypted)
}
